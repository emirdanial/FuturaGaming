  ...
  <div class="container">
    ...
    <div class="col-md-9">
      ...
      try {
        ...
        $stmt = $conn->prepare("SELECT * FROM tbl_products_a159753_pt2  ORDER BY fld_product_price DESC LIMIT $start_from, $per_page");
        $stmt->execute();
    ...
    <div class="row">
      ...
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_products_a159753_pt2 ORDER BY fld_product_price DESC");
            $stmt->execute();
            $result = $stmt->fetchAll();
          ...
          <?php } else { ?>
            <li><a href="cataloguehightolow.php?page=<?php echo $page-1 ?>" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
          <?php
          }
          for ($i=1; $i<=$total_pages; $i++)
            if ($i == $page)
              echo "<li class=\"active\"><a href=\"cataloguehightolow.php?page=$i\">$i</a></li>";
            else
              echo "<li><a href=\"cataloguehightolow.php?page=$i\">$i</a></li>";
          ?>
          <?php if ($page==$total_pages) { ?>
            <li class="disabled"><span aria-hidden="true">»</span></li>
          <?php } else { ?>
            <li><a href="cataloguehightolow?page=<?php echo $page+1 ?>" aria-label="Previous"><span aria-hidden="true">»</span></a></li>
          <?php } ?>
        </ul>
      </nav>
    </div>
